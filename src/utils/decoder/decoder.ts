...
function readVersion(matrix: BitMatrix): Version | undefined | null {
  const dimension = matrix.height;
  ...
  for (const version of VERSIONS) {
    ...
    let difference = numBitsDiffering(topRightVersionBits, (version as any).infoBits);
    if (difference < bestDifference) {
      bestVersion = version;
      bestDifference = difference;
    }

    difference = numBitsDiffering(bottomLeftVersionBits, (version as any).infoBits);
    if (difference < bestDifference) {
      bestVersion = version;
      bestDifference = difference;
    }
...
function getDataBlocks(codewords: number[], version: Version, ecLevel: number) {
  ...
  for (let i = 0; i < shortBlockSize; i++) {
    for (const dataBlock of dataBlocks) {
      dataBlock.codewords.push((codewords as any).shift());
    }
  }
  ...
  if (ecInfo.ecBlocks.length > 1) {
    ...
    for (let i = 0; i < largeBlockCount; i++) {
      dataBlocks[smallBlockCount + i].codewords.push((codewords as any).shift());
    }
  }
